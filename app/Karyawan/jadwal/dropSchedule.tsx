...
const DropSchedule = (myprop: props) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            // Hit endpoint to delete schedule
            ...
            if (response.data.success === true) {
                toast(message, {
                    containerId: `toastDrop-${myprop.schedule.id}`  ,
                    type: "success",
                });
                setShow(false);
                // Refresh page
    ...
    return (
        <div>
            <ToastContainer containerId={`toastDrop-${myprop.schedule.id}`} />
            <button type="button"
                onClick={() => openModal()}
                className="px-2 py-1 rounded-md bg-lime-300 hover:bg-pink-500 text-slate-950 flex flex-wrap ">
